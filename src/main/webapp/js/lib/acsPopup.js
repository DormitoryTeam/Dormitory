...
var acsPopup = $.extend({}, {
	...
	drawPopup: function(setting) {
		...
		var container = "<div class='"+ setting.popupLayerBg +"'></div>";
			container += "<iframe class='"+ setting.popupLayerBg +"' frameborder='no'></iframe>";
	...
	popupPosition:function(setting, isResize) {
		if (setting.popupScroll) $('html').css('overflow', 'hidden');
	...
	setPopupSize:function(setting, isreSize){
		...
		if(setting.popupMethod.toLowerCase() == 'fade'){
			...
			popupCov.animate({
				...
				if(setting.afterSetPosition) $.extend({}, setting).afterSetPosition(setting);
			});
		}else {
			popupCov.animate({
				'width' : setting.realW + setting.conMarginX + setting.covBorderX + setting.covPaddingX,
	...
	bindEvent:function(setting){
		if(setting.callBack) $.extend({}, setting).callBack(setting);
	...
	popupLayerClose: function(setting) {
		...
		$('.' + setting.popupLayerCov).animate({
			...
			$(this).detach();
			if (setting.popupScroll) $('html').css('overflow','');
	...
	popupMove: function(e, setting){
		...
		if(top <= setting.popupMargin + scrollY) top = setting.popupMargin + scrollY;
