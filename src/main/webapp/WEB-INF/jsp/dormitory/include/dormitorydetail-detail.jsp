<div class="row house-detail-info">
	...
	<div class="house-text-info">
		...
		<p><span style="font-weight: bolder">附加费用：</span><span><c:if test="${dormitory['additionalPrice']>=0}"><em>&#163;</em>&nbsp;<fmt:formatNumber value="${dormitory['additionalPrice']}" pattern="#0.00"/></c:if><c:if test="${dormitory['additionalPrice']<0}">暂未定价</c:if></span><c:if test="${not empty dormitory['additionalPriceText']}">, <span style="">${dormitory['additionalPriceText']}</span></c:if></p>
	</div>
